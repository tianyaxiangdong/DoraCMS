...
//对象新增
    router.post('/manage/:defaultUrl/addOne', function (req, res, next) {
        ...
        if (adminFunc.checkAdminPower(req, currentPage + '_add')) {
            ...
            } else if (targetObj == Config) {
                addOneConfig(req, res);
            } else {
                DbOpt.addOne(targetObj, req, res);
            }
        } else {
    ...
    function addOneConfig(req, res) {
        var newObj = new Config(req.body);
        Config.update({'_id':req.body._id}, newObj, {'upsert': true}, function (err) {
            if (err) {
                console.info(err);
                res.end(err);
            } else {
...
//文件管理界面（list）
    ...
    router.get('/manage/modifyConfig', function (req, res, next) {

        adminFunc.renderToManagePage(req, res, 'manage/modifyConfig', settings.MODIFYCONFIG);

    });
    router.get('/manage/modifyConfig/list', function (req, res, next) {

        Config.find({}, function (err,result) {
            if(err){
                res.next(err);
            }else{
                console.log("get modify config success!");
